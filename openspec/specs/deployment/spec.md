...
`014` MUST align `studio_instructors.email` to linked `users.email` where `user_id` is set and values diverge (data repair only; no schema change).

`015` MUST make `studio_bookings.pack_id` **nullable** (admin calendar enroll without pack) and MUST align `studio_students.email` to linked `users.email` where `user_id` is set and values diverge.

`016` MUST drop pack/fixed-enrollment schema (`studio_fixed_enrollments`, `studio_bookings.pack_id`, `studio_student_packs`, `studio_pack_products`) and MUST create arancel/abono tables plus nullable `studio_bookings.abono_id`.

If a restored dump still reports an orphan revision `003` from a **discarded** earlier MP reconciliation attempt that is **not** the current `003_mp_accounts` in the repo, operators MUST clean that revision before upgrade (drop orphan MP tables if present and stamp to a known good revision). New installs MUST only apply migrations present in the shipped image.
...
#### Scenario: Upgrade 015 nullable pack and student email align
- **GIVEN** a database at head `014`
- **WHEN** `015` applies
- **THEN** `studio_bookings.pack_id` MUST allow NULL
- **AND** linked student profile emails MUST match `users.email` where they diverged
...
- **WHEN** `016` applies
- **THEN** pack product/student pack/fixed enrollment tables MUST be gone
- **AND** `studio_bookings.abono_id` MUST allow NULL
- **AND** `studio_aranceles` / `studio_abonos` MUST exist
...
The backend Docker image MUST include `postgresql-client-18` (or compatible PostgreSQL client matching server major) to enable database dump creation (`pg_dump`) from within the application container.
...
- **THEN** the command MUST be executable without missing binary or version mismatch errors
